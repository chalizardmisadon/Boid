class Predator extends Boid {
  ...
  void buildBoid(float x, float y, int c){ // function setting
    ...
    size = random(0.8, 1.2);
    colour = 255 - colour + int(random(10))*15;
  }
  
  void move(ArrayList<Boid> prey){
    applyForce(prey);
  ...
  void applyForce(ArrayList<Boid> prey){ // hunting for predator
    // Cohesion acceleration
    acceleration = calcForce(cohesion(predator.flock));
    acc.add(acceleration.mult(0.2)); // Weight the force
    
    // Alignment acceleration
    acceleration = calcForce(alignment(predator.flock));
    acc.add(acceleration.mult(0.5)); // Weight the force
    
    // Separation acceleration
    acceleration = calcForce(separation(predator.flock));
    acc.add(acceleration.mult(4)); // Weight the force
    
    // Hunting acceleration
    acceleration = calcForce(hunt(prey));
  ...
  PVector hunt(ArrayList<Boid> prey){ // goes for group center of mass
